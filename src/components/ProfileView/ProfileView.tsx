    ...
    (acceptedFiles) => {
      ...
      reader.onload = () => {
        try {
          ...
          console.error(err);
          toast.error(
            `Could not load JSON Resume file :( Please make sure it's the valid format`
          );
        }
      };
    ...
    <div className="wizard-view profile-view special-bg" {...getRootProps()}>
      ...
      <div className="wrapper">
        ...
        <div className="profile-wrapper">
          ...
          <div
            className={`profile-edit-drop-msg ${
              uploadedName ? "post-upload" : ""
            }`}
          >
            {uploadedName ? (
              `${uploadedName}.json`
              ...
              <>
                <button onClick={open}>Click here</button> to upload your JSON
                file or just drop it anywhere on this page
              </>
            )}
          </div>
          <p>
            * CV Boostifier supports the{" "}
            <a
          ...
          <div className="buttons-wrapper">
            <Link
              className={`button primary ${!uploadedName ? "disabled" : ""}`}
              to={"/wizard/skins"}
            >
              Show me the money
            </Link>
            <Link className="button" to={"/wizard/skins"}>
              I prefer the hard way, without data
            </Link>
          </div>
        </div>
        <div className="file-upload-wrapper">
          <input {...getInputProps()} />
          {isDragActive && (
            <div className="drag-active">Drop your JSON Resume here...</div>
          )}
        </div>
      </div>
    </div>
